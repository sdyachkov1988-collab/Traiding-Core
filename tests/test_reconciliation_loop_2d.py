...
def _request_operator_from_protocol_surface(
    coordinator: RecoveryCoordinatorProtocol,
...
def test_recovery_coordinator_protocol_surface_carries_operator_command_request() -> None:
    coordinator = RecoveryCoordinator(
        source_of_truth=SourceOfTruthPolicy(),
        classifier=UnknownStateClassifier(),
    )

    request = _request_operator_from_protocol_surface(coordinator, "btc-usdt")
